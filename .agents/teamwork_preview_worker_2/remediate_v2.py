...
for line in lines:
    ...
    if in_products and 'const CATEGORIES = [' in line:
        in_products = False
        new_lines.append(line)
        continue
    
    if in_products:
        # Check start of product object
        stripped = line.strip()
        ...
        if in_product_obj:
            ...
            # Discard any imageUrl line inside product object
            if 'imageUrl:' in line:
                continue
            
            current_obj_lines.append(line)
            
            # Check end of product object (closing brace at outer object level)
            if stripped == '}' or stripped == '},':
                in_product_obj = False
                products_count += 1
                # Format current_obj_lines: insert single imageUrl after id
                final_obj_lines = []
                inserted = False
                for l in current_obj_lines:
                    final_obj_lines.append(l)
                    if 'id:' in l and current_id and not inserted:
                        indent = l[:len(l) - len(l.lstrip())]
                        new_url = URL_MAP.get(current_id)
                        if not new_url:
                            print(f"ERROR: No URL mapping for {current_id}")
                        final_obj_lines.append(f"{indent}imageUrl: '{new_url}',\n")
                        inserted = True
                new_lines.extend(final_obj_lines)
                current_obj_lines = []
                current_id = None
                continue
            continue
...
print(f"Processed {products_count} product objects.")

full_content = ''.join(new_lines)

# Also update CATEGORY_CDN_FALLBACKS, getCategorySvgDataUri, handleImageError
...
full_content = re.sub(r"const\s+CATEGORY_CDN_FALLBACKS\s*=\s*\{[\s\S]*?\};", new_category_cdn_fallbacks, full_content, count=1)
...
full_content = re.sub(r"function\s+getCategorySvgDataUri\([\s\S]*?\n\s*\}", new_get_category_svg, full_content, count=1)
...
full_content = re.sub(r"function\s+handleImageError\([\s\S]*?\n\s*\}", new_handle_image_error, full_content, count=1)

with open(html_path, 'w', encoding='utf-8') as f:
    f.write(full_content)

print("Finished remediate_v2.py execution!")
